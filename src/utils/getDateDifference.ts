...
function getFormatedStringFromDays(numberOfDays: number) {
  ...
  const yearsDisplay = years ? years + (years === 1 ? ' year' : ' years') : '';
  const monthsDisplay = months
    ? months + (months === 1 ? ' month' : ' months')
    : '';

  const ymSep = yearsDisplay && (months || days) ? ', ' : '';
  const mdSep = (yearsDisplay || monthsDisplay) && days ? ', ' : '';
  return `${yearsDisplay}${ymSep}${monthsDisplay}${mdSep}${daysDisplay}`;
}
